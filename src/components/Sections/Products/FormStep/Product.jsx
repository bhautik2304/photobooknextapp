...
import { formError, formNext,selectProduct } from '../../../../Redux/Slice/orderSlice'

function Product() {
    const { product: { product }, order } = useSelector(state => state)
            ...
            {product ? product.map((data, key) => (
                <>
                    <div key={key} className="col-lg-3 col-md-3 col-sm-12">
                        <div className={`pro ${(order.product?.id == data.id) ? 'selected_prod' : null}`} onClick={() => disapatch(selectProduct({ product: data }))} >
